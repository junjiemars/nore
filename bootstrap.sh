...
echo "check_cc_include () {"
echo "  local inc_list=\"\\${1:-\\${ROOT}/.cc-inc.list}\";"
echo "  local inc_list=\"\${1:-\\${ROOT}/.cc-inc.list}\";"
echo "  command -v cc &>/dev/null || return 1"
echo "  local cc_out=\\"\\$(echo '' | cc -v -E 2>&1 >/dev/null - | awk '/#include <...> search starts here:/,/End of search list./')\\";"
echo "  [ -n \"\\$cc_out\" ] || return 1;"
echo "  local inc_lns=();"
echo "  IFS='\\n';"
echo "  for l in $(echo \"\\$cc_out\"); do"
echo "    inc_lns+=(\\$(echo \"\\$l\" | sed 's/^ //'));"
echo "  done;"
echo "  unset IFS;"
echo "  cat /dev/null > \"\\$inc_list\";"
echo "  local inc_ln=\"\\${#inc_lns[@]}\";"
echo "  if [[ 2 -lt \"\\$inc_ln\" ]]; then"
echo "    local inc_paths=(\"\\${inc_lns[@]:1:\\$(( inc_ln-2  ))}\");"
echo "    echo \"\\${inc_paths[@]}\" >> \"\\$inc_list\";"
echo "  fi;"
echo "}"
...
if test ! -f "\${ROOT}/.cc-checked.id" || test "0" = "\`cat \${ROOT}/.cc-checked.id\`"; then
  check_cc_include \${ROOT}/.cc-inc.list
  echo "1" > "\${ROOT%/}/.cc-checked.id"
fi
