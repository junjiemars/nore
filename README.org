...
On Unix-like operating systems, there are no more thing need to do,
and . But on Windows, if using *MSVC* environment, we need host *MSVC*
environment first.
...
- =dumb= will run =nm_feature_test= except does not output to screen.
