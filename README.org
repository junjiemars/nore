...
*Why another C build system?*

I often need to experiment with low-level C code across platforms, yet lack
simple tools for this purpose. While existing solutions like /Make/ and /Shell/
address core needs, the critical challenge remains: /how to simplify *multi-OS*
and *multi-compiler* workflows without sacrificing flexibility./

*Nore's approach*:
- Preserve Unix simplicity:
  maintain the classic *configure-make* workflow.
- Unify development cycles:
  *write*, *build*, *test*, and *run* seamlessly on Linux, Darwin, and Windows.
- Enable rapid experimentation:
  support *prototyping* without imposing unnecessary abstractions.
...
Using *new* command to make a testable skeleton, you can easy to try
